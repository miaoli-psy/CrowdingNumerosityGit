...
def endPractice():
    ...
    end_practice2.setText(
        'There are 10 blocks of the real experiment, you will see 3 reference images before each block.')
    ...
    if keypress[0] == 'escape':
        core.quit()
def startBlock(ref_image1, ref_image2,ref_image3, Number1, Number2, Number3):
...
def runTrial(training=False, trialInfo=None,nFrames=10, strictResponse=True, blockNo=None):
    ...
    if keypress[0] == 'escape':
        core.quit()
    if training:
        image.setImage(u'2_c_2_f_100_wS_0.4_eS_0.15811388300841897_0.15811388300841897_33.png')
    else:
    ...
    while not done:
        captured_stringbottom, pcaptured_stringbottom, bigLetter, twoLetter = returnNewString(captured_stringbottom,
                                                                                              pcaptured_stringbottom)
    ...
    with open(alternative_filename, 'a') as f:
        line = "%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;\n" % (
            captured_stringbottom, pk1, rt1, training, trialInfo,nFrames, strictResponse, blockNo,expInfo['expName'],expInfo['blockOrder'])
        f.write(line)
    event.clearEvents()
...
alternative_filename = _thisDir + os.sep + u'data_Crwdng_Nmrsty1/alternative_group_%s_participant_%s_date_%s' % (expInfo['group'], expInfo['participant'], expInfo['date'])
with open(alternative_filename, 'a') as f:
    firstline = "%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;\n" % ('subjID', 'session',
                                                                                       'blockNo', 'trialno', 'rt1',
                                                                                       'correct1', 'pk1', 'corransw',
                                                                                       'helperSpacing',
                                                                                       'flankerSpacing', 'crowded',
                                                                                       'helper', 'targetO',
                                                                                       'flankerstr', 'leftObjParams',
                                                                                       'rightObjParams',
                                                                                       'downObjParams', 'presentedTo',
                                                                                       'contrast', 'helperOri',
                                                                                       'flankerCategory')
    f.write(firstline)
...
for b in range(startingBlockNumber,len(blocks)):
    ...
    startBlock(blocks['ref_image1'][b], blocks['ref_image2'][b], blocks['ref_image3'][b],
               blocks['Number1'][b], blocks['Number2'][b], blocks['Number3'][b])
