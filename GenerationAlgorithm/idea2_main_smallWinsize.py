...
import idea2_direcMani_smallWinsize
...
# =============================================================================
start = time.time()
multiParaFunc = partial(idea2_direcMani_smallWinsize.runStimuliGeneration, newWindowSize, drawEllipseFig, ellipse_ka, ellipse_kb) 
...
# tqdm task time Bar: https://github.com/tqdm/tqdm/issues/484
    ...
    for _ in tqdm(pool.imap_unordered(multiParaFunc, range(11,runN)), total=runN):#range(1,50) runs 49 times from 1 to 49
        pass
    # multicore()
    pool.close()
    pool.join()
